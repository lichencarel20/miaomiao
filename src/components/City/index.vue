  ...
  <div class="city_body">
    <div class="city_list">
      <div class="city_hot">
        ...
        <ul class="clearfix">
          <li
            v-for="data in origin"
            :key="data.index"
            @click="handleSet(data.name,data.cityId)"
          >{{data.name}}</li>
        </ul>
      </div>
      <div class="city_sort" ref="city_sort">
        <div v-for="data in citylist" :key="data.index">
          ...
          <ul v-for="jink in data.list" :key="jink.cityId">
            <li @click="handleSet(jink.name,jink.cityId)">{{jink.name}}</li>
          </ul>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    // 通过动态管理，动态衔接城市转换至相应城市的电影页面
    handleSet(nm, id) {
      console.log(nm, id);
      var activated = this.activated;
      console.log(activated);
      // 通过LOCALSTRORAGE记录上次点击的城市，作为nowplaying页面的默认设置，实现动态衔接
      window.localStorage.setItem("cityNm", nm);
      window.localStorage.setItem("cityId", id);

      this.$store.commit("city/ACTIVATED", { activated });
  ...
  mounted() {
    var origin = window.localStorage.getItem("hotCity");
    ...
    if (origin && citylist) {
      ...
      axios({
        ...
        headers: {
          "X-Client-Info":
            '{"a":"3000","ch":"1002","v":"5.0.4","e":"159551147410496900071431"}',
        ...
        .then((res) => {
          ...
          window.localStorage.setItem("hotCity", JSON.stringify(this.origin));
