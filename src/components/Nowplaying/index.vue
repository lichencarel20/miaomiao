<template>
	<div>
		<loading v-if="isShow" />
		<div v-else class="movie_body" :style="PH">
			<Scroller>
					<ul >
						<li v-for="data in datalist" :key="data.filmId" @tap="godetail(data.filmId)">
							...
							<div class="info_list">
								...
								<p>主演 <span v-for="item in data.actors" :key="item.length">{{item.name}} ,</span></p>
...
export default {
	...
	activated(){
		// 解决keep alive导致mounted生命周期无法执行的问题，通过局部变量赋值，对比状态管理，动态解决CITYID
		...
		this.isShow=true
		this.axios({
			url:`https://m.maizuo.com/gateway?cityId=${cityId}&pageNum=1&pageSize=10&type=1&k=7262748`,
			headers:{
